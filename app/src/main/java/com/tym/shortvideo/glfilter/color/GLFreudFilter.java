...
public class GLFreudFilter extends GLImageFilter {
    ...
    private int mRandTextureLoc;

    private int mStrengthLoc;
    private int mInputTextureWidth;
    private int mInputTextureHeight;

    public GLFreudFilter() {
        this(VERTEX_SHADER, FRAGMENT_SHADER);
    }

    public GLFreudFilter(String vertexShader, String fragmentShader) {
        super(vertexShader, fragmentShader);

        mRandTextureLoc = GLES30.glGetUniformLocation(mProgramHandle, "randTexture");
        mStrengthLoc = GLES30.glGetUniformLocation(mProgramHandle, "strength");
        mInputTextureWidth = GLES30.glGetUniformLocation(mProgramHandle, "inputTextureWidth");
        mInputTextureHeight = GLES30.glGetUniformLocation(mProgramHandle, "inputTextureHeight");
        createTexture();
        setFloat(mStrengthLoc, 1.0f);
    }

    private void createTexture() {
        mRandTexture = GlUtil.createTextureFromAssets(ParamsManager.context,
    ...
    public void onInputSizeChanged(int width, int height) {
        ...
        setFloat(mInputTextureWidth, 1.0f / mImageWidth);
        setFloat(mInputTextureHeight, 1.0f / mImageHeight);
    }
    ...
    public void onDrawArraysBegin() {
        ...
        GLES30.glUniform1i(mRandTextureLoc, 1);
    }
