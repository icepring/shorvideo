package com.tym.shortvideo.filter;
...
import com.tym.shortvideo.glfilter.base.SlideGpuFilterGroup;
import com.tym.shortvideo.glfilter.group.GLDefaultFilterGroup;
import com.tym.shortvideo.media.VideoInfo;
import com.tym.shortvideo.tymtest.MatrixUtils;
...
public class VideoDrawer implements GLSurfaceView.Renderer {
     ...
     * 美白的filter
     */
    private GLDefaultFilterGroup mBeautyFilter;
     ...
     * 绘制其他样式的滤镜
     */
    private GLImageFilter mGroupFilter;
    /**
     * 控件的长宽
     */
    ...
    public VideoDrawer(Context context, Resources res) {
        ...
        mBeautyFilter = new GLDefaultFilterGroup();
...
//        mShow.setMatrix(OM);
    ...
    public void onSurfaceChanged(GL10 gl, int width, int height) {
        ...
        mBeautyFilter.onDisplayChanged(viewWidth, viewHeight);
    ...
    public void onDrawFrame(GL10 gl) {
        ...
        if (mBeautyFilter != null && isBeauty && mBeautyFilter.getBeauty() != 0) {
            GlUtil.bindFrameTexture(fFrame[0], fTexture[0]);
            GLES20.glViewport(0, 0, viewWidth, viewHeight);
//            mBeautyFilter.drawFrame(mBeFilter.getOutputTexture());
            GlUtil.unBindFrameBuffer();
        ...
        mSlideFilterGroup.drawFrame(mProcessFilter.getOutputTexture());
        if (mGroupFilter != null) {
            GlUtil.bindFrameTexture(fFrame[0], fTexture[0]);
            GLES20.glViewport(0, 0, viewWidth, viewHeight);
            mGroupFilter.drawFrame(mSlideFilterGroup.getOutputTexture());
            GlUtil.unBindFrameBuffer();
            mProcessFilter.setTextureId(fTexture[0]);
        } else {
    ...
    public void setGpuFilter(GLImageFilter filter) {
        if (filter != null) {
            mGroupFilter = filter;
            mGroupFilter.onDisplayChanged(viewWidth, viewWidth);
