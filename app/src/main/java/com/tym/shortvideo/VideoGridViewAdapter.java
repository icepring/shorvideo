...
public class VideoGridViewAdapter extends RecyclerView.Adapter<VideoGridViewAdapter.VideoViewHolder> {
    ...
    public void onBindViewHolder(VideoViewHolder holder, int position) {
        ...
        Glide.with(MyApplication.getContext()).load(TrimVideoUtil.getVideoFilePath(video.getPath())).into(holder.videoCover);
...
//        holder.videoCover.setImageBitmap(bitmap);
    ...
    class VideoViewHolder extends RecyclerView.ViewHolder {
        ...
        VideoViewHolder(final View itemView) {
            ...
            int size = DeviceUtils.getScreenWidth() / 4;
                ...
                public void onClick(View v) {
                    ...
                    if(videoSelect.size() > 0 ){
                        if(videoInfo.equals(videoSelect.get(0))){
                            selectIcon.setImageResource(R.mipmap.icon_video_unselected);
                            clearAll();
                            isSelected = false;
                        }else{
                            selectIconList.get(0).setImageResource(R.mipmap.icon_video_unselected);
                            clearAll();
                            addData(videoInfo);
                            selectIcon.setImageResource(R.mipmap.icon_video_selected);
                            isSelected = true;
                        }
